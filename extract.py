...
Original file is located at
    https://colab.research.google.com/drive/1doK8Ph4PTOtGETBRZatmne2UuESxHGtv
"""
...
model_extracting = CustomExtractingFRCNN(
    backbone,
    num_classes=91,
...
model_predicting = CustomPredictingFRCNN(
    backbone,
    num_classes=91,
    # rpn_anchor_generator=anchor_generator,
    # rpn_head=rpn_head,
...
if weights is not None:
    model_extracting.load_state_dict(weights.get_state_dict(progress=True, check_hash=True))
    model_predicting.load_state_dict(weights.get_state_dict(progress=True, check_hash=True))
    if weights == FasterRCNN_ResNet50_FPN_Weights.COCO_V1:
            overwrite_eps(model_extracting, 0.0)
            overwrite_eps(model_predicting, 0.0)
# model_extracting.mod_roi_heads()
# model_predicting.mod_roi_heads()
...
# save_path = '/content/drive/MyDrive/UIT_UNIVERSITY/NCKH/FEATURES/fc7_weights'
# np.save(os.path.join(save_path, 'fc7_weight.npy'), model_torch.roi_heads.box_head.fc7.state_dict()['weight'])
# np.save(os.path.join(save_path, 'fc7_bias.npy'), model_torch.roi_heads.box_head.fc7.state_dict()['bias'])

"""# **RUN INFERENCE**"""

def get_features_results(model_extracting, model_predicting, list_images):
    # list_images = []
    # for img_path in img_path_list:
    #     img, im_scale, im_info = _image_transform(img_path)
    #     list_images.append(torch.tensor(img))

    # Extracting
    model_extracting.eval()
    box_features, features, proposals, proposal_losses, images, original_image_sizes = model_extracting(list_images)

    # Predictor
    model_predicting.eval()
    result, class_logits, box_regression = model_predicting(
        images=images,
        features=features,
        proposals=proposals,
    ...
    # Pred_boxes
    pred_boxes_logits = model_predicting.roi_heads.box_coder.decode(box_regression, proposals)
...
# # Load an Image
# img_path = img_image_paths[1]
# img, im_scale, im_info = _image_transform(img_path)

# # Extract Features
# model_extracting.eval()
# box_features, features, proposals, proposal_losses, images, original_image_sizes = model_extracting(
...
# pred_boxes_logits = model_predicting.roi_heads.box_coder.decode(box_regression, proposals)
# pred_boxes_logits.shape

# get_features_results(model_extracting, model_predicting, img_path_list)

# img_path_list = [img_image_paths[2], img_image_paths[1]]
# box_features, class_logits, proposals, pred_boxes_logits = get_features_results(model_extracting, model_predicting, img_path_list)
...
class FeatureExtractor:
    ...
    def __init__(self):
        ...
        self.model_extracting = model_extracting.to(self.device)
        self.model_predicting = model_predicting.to(self.device)
    ...
    def get_detectron_features(self, image_paths):
        ...
        with torch.inference_mode():
            ...
            box_features, class_logits, proposals, pred_boxes_logits = get_features_results(
                model_extracting=self.model_extracting,
                model_predicting=self.model_predicting,
    ...
    def extract_features(self):
        ...
        if os.path.isfile(image_dir):
            ...
            # Extracting
            # for iter, chunk in tqdm(enumerate(self._chunks(files, self.args.batch_size))):
            #     features, infos = self.get_detectron_features(chunk)
            #     for idx, file_name in enumerate(chunk):
            #         name = self._save_feature(file_name, features[idx], infos[idx])
            #         tqdm.write(f"Saving: {name}.npy")
            ic(len(files))
